//
//  RRFeedInfoModel.m
//  rework-reader
//
//  Created by 张超 on 2019/2/14.
//  Copyright © 2019 orzer. All rights reserved.
//

#import "RRFeedInfoModel.h"
#import "RPKeyAndValueModelProtocol.h"

@interface RRFeedInfoModel () <RPKeyAndValueModelProtocol>

@end

@implementation RRFeedInfoModel

@synthesize entityUUID;

@synthesize entityKey;

@synthesize entityType;

@synthesize entityValue;

- (id)entityKey
{
    return self.key;
}

- (id)entityValue
{
    if (self.type == RRFeedInfoTypeSwitch) {
        return self.switchValue;
    }
    return self.origin_value;
}

@end
